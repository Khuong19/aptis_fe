'use client';

import FilterBar from '@/app/components/ui/FilterBar';

interface QuestionBankFiltersProps {
  searchTerm: string;
  setSearchTerm: (value: string) => void;
  typeFilter: 'all' | 'ai-generated' | 'official';
  setTypeFilter: (value: 'all' | 'ai-generated' | 'official') => void;
  partFilter: 'all' | '1' | '2' | '3' | '4';
  setPartFilter: (value: 'all' | '1' | '2' | '3' | '4') => void;
  accessFilter: 'all' | 'public' | 'private';
  setAccessFilter: (value: 'all' | 'public' | 'private') => void;
}

const QuestionBankFilters = ({
  searchTerm,
  setSearchTerm,
  typeFilter,
  setTypeFilter,
  partFilter,
  setPartFilter,
  accessFilter,
  setAccessFilter
}: QuestionBankFiltersProps) => {
  const filters = [
    {
      key: 'type',
      label: 'Type',
      options: [
        { value: 'all', label: 'All Types' },
        { value: 'ai-generated', label: 'AI-Generated' },
        { value: 'official', label: 'Official' }
      ],
      value: typeFilter,
      onChange: setTypeFilter
    },
    {
      key: 'part',
      label: 'Part',
      options: [
        { value: 'all', label: 'All Parts' },
        { value: '1', label: 'Part 1' },
        { value: '2', label: 'Part 2' },
        { value: '3', label: 'Part 3' },
        { value: '4', label: 'Part 4' }
      ],
      value: partFilter,
      onChange: setPartFilter
    },
    {
      key: 'access',
      label: 'Access Type',
      options: [
        { value: 'all', label: 'All Access Types' },
        { value: 'public', label: 'Public' },
        { value: 'private', label: 'Private' }
      ],
      value: accessFilter,
      onChange: setAccessFilter
    }
  ];

  return (
    <FilterBar
      searchTerm={searchTerm}
      setSearchTerm={setSearchTerm}
      filters={filters}
      searchPlaceholder="Search question sets..."
    />
  );
};

export default QuestionBankFilters;
